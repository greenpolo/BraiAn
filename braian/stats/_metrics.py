...
def _hemisphered_metric(brain: AnimalBrain,
    ...
    for marker in brain.markers:
        ...
        for sizes in brain.hemisizes:
            ...
            data: BrainData = metric(sizes, sizes.hemisphere, marker_data, marker)
            data._metric = metric_str
...
def _group_change(brain: AnimalBrain, group: AnimalGroup,
    ...
    group_hemimean: AnimalBrain = group.mean()
    def group_metric(sizes: BrainData, hem: BrainHemisphere,
        ...
        group_mean = group_hemimean[marker,hem]
        brain_sizes = brain[hem]
        group_mean = group_mean.select_from_data(brain_sizes, fill_nan=True, inplace=False)
        # NOTE: `select_from_data` needs an equivalent BrainData to trust the list of regions.
...
def markers_correlation(marker1: str, marker2: str,
    ...
    if other is None:
        ...
        _compatibility_check((group, other))
